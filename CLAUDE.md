...
- Title: *From Directing to Dialogue: 15 Weeks of AI Agent Coding for Performance Engineering*. Subtitle: *Two projects · 7 sessions · ~1,500 prompts*. The "directing → dialogue" framing is the thesis the rest of the deck builds on; CCS slides set up "directing", rust-gpu slides set up "dialogue". 15 weeks = Feb 25 → Jun 8 (CCS start to last claspr commit), not just the rust-gpu portion.
